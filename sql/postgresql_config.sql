...
CREATE TYPE sip_transport_values AS ENUM ('udp','tcp','tls','ws','wss','udp,tcp','tcp,udp','tls,udp,tcp','tls,udp','tls,tcp','ws,udp,tcp,tls','wss,udp,tcp,tls');
...
CREATE TABLE sip_peers (
    ...
    transport sip_transport_values default 'wss,udp,tcp,tls', 
    dtmfmode sip_dtmfmode_values, 
    directmedia sip_directmedia_values, 
    nat VARCHAR(29) default 'auto_force_rport,auto_comedia', 
    ...
    fromuser VARCHAR(40), 
    qualify VARCHAR(40) not null default 'yes', 
    ...
    textsupport yes_no_values not null default 'yes', 
    ...
    supportpath yes_no_values,
		comment character varying (80) not null default '', 
		encryption yes_no_values default 'yes' not null,
		avpf yes_no_values default 'no' not null,
    PRIMARY KEY (id), 
    UNIQUE (name),
...
CREATE INDEX sip_peers_name_host ON sip_peers (name, host);

CREATE INDEX sip_peers_ipaddr_port ON sip_peers (ipaddr, port);

CREATE INDEX sip_peers_host_port ON sip_peers (host, port);
...
ALTER TABLE sip_peers ALTER COLUMN useragent TYPE VARCHAR(255);
...
INSERT INTO alembic_version (version_num) VALUES ('a541e0b5e89');
