...
#         FILE:  Module.pm
#
#  DESCRIPTION:  Base class for PearlPBX Modules. 
#
#        NOTES:  ---
...
=head1 SYNOPSIS
	...
	use base PearlPBX::Module;

=head1 DESCRIPTION

C<PearlPBX::Module> module contains superclass all other classes should be inherited from.

=cut

package PearlPBX::Module;
...
=item B<new($configfilename)> - class constructor
    ...
    my $object = PearlPBX::Module->new(%options);
...
sub run { 
  ...
  my $subname = $params->{'sub'};

  if ( $this->can($subname) ) {
    return $this->$subname($params);
  }
