...
package PearlPBX::Module::WBList; 
...
sub _tableName { 
	...
	return undef unless ( defined ( $wb ) );

	if ( $wb =~ /black/ ) { 
		return 'public.blacklist'; 
	}
	if ( $wb =~ /white/ ) {
		return 'public.whitelist'; 
	}
...
sub getJSON {
	...
	my $sql = "select * from ".$tableName." order by number"; 
