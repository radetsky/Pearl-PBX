...
sub report {
    ...
    my $sql =
		"select calldate,src,dst,split_part(channel,'-',1) as channel, split_part(dstchannel,'-',1) as dstchannel,disposition,billsec from public.cdr where calldate between ? and ? and split_part(dstchannel,'-',1) in (select interface from public.queue_members where queue_name = ?) order by calldate";
