#===============================================================================
#
#         FILE:  outgoingMadeByOperator.pm
#
#  DESCRIPTION:
#
#        NOTES:  ---
...
package PearlPBX::Report::outgoingMadeByOperator;
...
sub report {
    ...
    my $sql =
		"select calldate,src,dst,split_part(channel,'-',1) as channel, split_part(dstchannel,'-',1) as dstchannel,disposition,billsec,uniqueid from public.cdr where calldate between ? and ? and src = ? order by calldate";
    ...
    unless ($hash_ref) {
		...
		$template->process ('outgoingMadeByOperator.html', $template_vars) || die $template->error();
