...
package PearlPBX::Report::LostInGroups1;
...
sub report {
    ...
    my $sql = "select count(queue) as s,queue as queuename from public.queue_parsed 
      where time between ? and ? and success=0 and callerid not in (select msisdn from ivr.addressbook ) 
      group by queue order by count(queue) desc"; 
   ...
   my $sth = $this->{dbh}->prepare($sql);
    ...
    foreach my $pos ( @cdr_keys ) { 
      ...
      tilldatetime => $tilldatetime,
		...
		$template->process('LostInGroups1.html', $template_vars) || die $template->error(); 
