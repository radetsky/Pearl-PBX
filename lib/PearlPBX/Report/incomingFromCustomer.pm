...
package PearlPBX::Report::incomingFromCustomer;
...
sub report {
    ...
    my $sql =
"select calldate,src,dst,split_part(channel,'-',1) as channel, split_part(dstchannel,'-',1) as dstchannel,disposition,billsec,uniqueid from public.cdr where calldate between ? and ? and src = ? and disposition='ANSWERED' order by calldate desc";
    ...
    unless ($hash_ref) {
		...
		$template->process ('incomingFromCustomer.html', $template_vars) || die $template->error();
