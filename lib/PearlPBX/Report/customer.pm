...
package PearlPBX::Report::customer;
...
sub report {
    ...
    my $sql =
"select calldate,src,dst,split_part(channel,'-',1) as channel, split_part(dstchannel,'-',1) as dstchannel,disposition,billsec,uniqueid from public.cdr where calldate between ? and ? and ( src = ? or dst = ? ) and disposition='ANSWERED' order by calldate desc";

    my $sth = $this->{dbh}->prepare($sql);
    eval { $sth->execute( $sincedatetime, $tilldatetime, $phone, $phone ); };
    if ($@) {
    ...
    unless ($hash_ref) {
		...
		$template->process ('customer.html', $template_vars) || die $template->error();
