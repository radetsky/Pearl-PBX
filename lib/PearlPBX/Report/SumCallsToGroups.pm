...
package PearlPBX::Report::SumCallsToGroups;
...
sub report {
    ...
    my $sql =
"select count(queuename) as s,queuename from queue_log where event='CONNECT' and time between ? and ? group by queuename order by count(queuename) desc"; 
    ...
    while ( my $hash_ref = $sth->fetchrow_hashref ) {
		...
		$template->process('SumCallsToGroups.html', $template_vars) || die $template->error(); 
