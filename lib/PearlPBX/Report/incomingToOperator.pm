...
package PearlPBX::Report::incomingToOperator;
...
sub report {
    ...
    my $tilldatetime  = $this->filldatetime( $params->{'dateTo'},  $params->{'timeTo'} );
		my $interface = 'SIP/'.$params->{'phone'};

    my $sql =
		"select calldate,src,dst,split_part(channel,'-',1) as channel, split_part(dstchannel,'-',1) as dstchannel,disposition,billsec,uniqueid from public.cdr where calldate between ? and ? and split_part(dstchannel,'-',1) = ? order by calldate";

    my $sth = $this->{dbh}->prepare($sql);
    eval { $sth->execute( $sincedatetime, $tilldatetime, $interface ); };
    if ($@) {
    ...
    unless ($hash_ref) {
		...
		$template->process ('incomingToOperator.html', $template_vars) || die $template->error();
