...
sub report {
    ...
    my $sql =
 "select count(agent) as s,agent as operator from public.queue_log where queuename = ? and event='CONNECT' and time between ? and ? group by agent order by count(agent) desc";
