#===============================================================================
#
#         FILE:  callsToExternalNumbers.pm
#
#  DESCRIPTION:
#
#        NOTES:  ---
...
package PearlPBX::Report::callsToExternalNumbers;
...
sub report {
    ...
    my $tilldatetime  = $this->filldatetime( $params->{'dateTo'},  $params->{'timeTo'} );
		my $dst = $params->{'extension'};

    my $sql =
"select calldate,src,dst,split_part(channel,'-',1) as channel, split_part(dstchannel,'-',1) as dstchannel,disposition,billsec,uniqueid from public.cdr where calldate between ? and ? and dst ~ ? order by calldate desc";

    my $sth = $this->{dbh}->prepare($sql);
    eval { $sth->execute( $sincedatetime, $tilldatetime, $dst ); };
    if ($@) {
    ...
    unless ($hash_ref) {
		...
		$template->process ('callsToExternalNumbers.html', $template_vars) || die $template->error();
